...
class ApiConfig {
  ...
  static const Duration scanTimeout = Duration(minutes: 15); // Increased for behavioral analysis
  ...
  static Future<void> setApiKey(SecurityProvider provider, String apiKey) async {
    final prefs = await SharedPreferences.getInstance();
    switch (provider) {
      case SecurityProvider.virusTotal:
  ...
  static Future<void> setPremiumAccount(SecurityProvider provider, bool isPremium) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<int> getMaxFileSizeForProvider(SecurityProvider provider) async {
    final isPremium = await isPremiumAccount(provider);
    return isPremium ? provider.maxFileSizePaid : provider.maxFileSizeFree;
  }
