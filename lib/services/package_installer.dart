...
class PackageInstaller {
  ...
  Future<InstallationResult> installFromUrl(String url,
    ...
    switch (package.source) {
      ...
      case PackageSource.github:
        if (package.downloadUrl != null) {
          return await installFromUrl(package.downloadUrl!,
  ...
  Future<InstallationResult> _installAptPackage(
      String packageName, DateTime startTime) async {
    try {
      ...
      // Install package
      final result =
          await Process.run('pkexec', ['apt', 'install', '-y', packageName]);
  ...
  Future<InstallationResult> _installSnapPackage(
      String packageName, DateTime startTime) async {
    try {
      final result =
          await Process.run('pkexec', ['snap', 'install', packageName]);
  ...
  Future<InstallationResult> _installFlatpakPackage(
      String packageName, DateTime startTime) async {
    try {
      final result = await Process.run(
          'pkexec', ['flatpak', 'install', '-y', 'flathub', packageName]);
  ...
  Future<InstallationResult> installPackage(String filePath,
      {bool skipVirusScan = false}) async {
    final startTime = DateTime.now();
  ...
  Future<InstallationResult> _installDebPackage(
      String filePath, DateTime startTime) async {
    ...
    try {
      ...
      final aptResult =
          await Process.run('pkexec', ['apt-get', 'install', '-f', '-y']);
  ...
  Future<InstallationResult> _installRpmPackage(
      String filePath, DateTime startTime) async {
    final StringBuffer output = StringBuffer();
    ...
    try {
      ...
      if (hasDnf) {
        result =
            await Process.run('pkexec', ['dnf', 'install', '-y', filePath]);
      } else if (hasYum) {
        result =
            await Process.run('pkexec', ['yum', 'install', '-y', filePath]);
      } else {
  ...
  Future<InstallationResult> _installPkgPackage(
      String filePath, DateTime startTime) async {
    final StringBuffer output = StringBuffer();
    ...
    try {
      ...
      final result = await Process.run(
          'pkexec', ['pacman', '-U', '--noconfirm', filePath]);
  ...
  Future<InstallationResult> _installTarPackage(
      String filePath, DateTime startTime) async {
    final StringBuffer output = StringBuffer();
    ...
    try {
      ...
      final extractResult =
          await Process.run('tar', ['-xf', filePath, '-C', tempDir.path]);
  ...
  Future<InstallationResult> _installAppImage(
      String filePath, DateTime startTime) async {
    final StringBuffer output = StringBuffer();
    ...
    try {
      ...
      try {
        final extractResult = await Process.run(
            destPath, ['--appimage-extract-and-run', '--appimage-help']);
        if (extractResult.exitCode == 0) {
    ...
    try {
      ...
      if (!await _securityScanner.isConfigured()) {
        ...
        return InstallationResult(
          ...
          output: '',
          error:
              '${provider.name} API key not configured. Please configure it in Settings.',
      ...
      if (!await _securityScanner.isFileEligibleForScanning(filePath)) {
        ...
        return InstallationResult(
          ...
          output: '',
          error:
              'File too large for scanning ($fileSize). Maximum size: ${_formatFileSize(maxSize)} for ${provider.name}.',
          installTime: Duration.zero,
        );
      }
      ...
      // Store scan report
      final storedReport =
          report.toStoredScanReport().copyWith(provider: report.provider);
      ...
      if (report.isMalicious) {
        return InstallationResult(
          ...
          error:
              'File flagged as malicious by ${report.provider.name}. Installation blocked for safety.',
          installTime: Duration.zero,
        );
        ...
        return InstallationResult(
          ...
          output: '${report.provider.name} Scan Results:\n'
              ...
              'Detected Threats:\n${report.detectedThreats.join('\n')}',
          error:
              'File flagged as suspicious by ${report.provider.name}. Please review before installation.',
          installTime: Duration.zero,
        );
      }
